...
const ALLOWED_HOSTS = new Set(['cuteinternet.com', 'emojidb.org'])

export async function collectWeb(targets, { delayMs = 2000, retries = 2, browserFactory = () => chromium.launch({ headless: true }) } = {}) {
  for (const target of targets) {
    const parsed = new URL(target.url)
    if (parsed.protocol !== 'https:' || !ALLOWED_HOSTS.has(parsed.hostname)) throw new Error(`目标不在允许列表: ${target.url}`)
