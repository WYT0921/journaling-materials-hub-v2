...
async function execute(trigger, dryRun = false) {
  const lockToken = randomUUID(); let runId = `dry-${Date.now()}`; const startedAt = new Date().toISOString(); const errors = []; let inserted = 0, backendDuplicates = 0, backendFiltered = 0, backendFailed = 0; let hasStartedRun = false
  const sources = []; const all = []
  try {
    if (!dryRun) { const startResult = await backend.startRun({ triggerType: trigger, lockToken }); runId = startResult.id; hasStartedRun = true }
    ...
    await writeReport(dataDir, runId, report)
    if (!dryRun && hasStartedRun) await backend.finishRun(runId, { ...report, sourceStats: sources, errorSummary: report.errors.join('; '), lockToken }).catch(() => {})
    throw error
  }
}
