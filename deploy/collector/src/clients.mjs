...
export class BackendClient {
  ...
  async request(path, options = {}) {
    const response = await this.fetch(`${this.baseUrl}${path}`, {
      ...
      headers: { 'Content-Type': 'application/json', 'X-Collector-Token': this.token, ...(options.headers || {}) }
    })
    const payload = await response.json().catch(() => ({}))
    if (!response.ok || payload.code && payload.code !== 200) throw new Error(payload.message || `Backend HTTP ${response.status}`)
    return payload.data
  }
