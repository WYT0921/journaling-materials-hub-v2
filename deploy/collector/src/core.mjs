...
const UI_ARTIFACT_PATTERN = /^(?:📋\s*copy|🔎|copy|copied!)$/iu
...
export function rejectionReason(value) {
  ...
  if (UI_ARTIFACT_PATTERN.test(content)) return 'ui_artifact'
  ...
  if (/^[\x00-\x7F]+$/u.test(content) && letters > 0) return 'plain_text'
...
export function buildCandidate(content, source, sourceUrl) {
  ...
  return {
    candidate: {
      ...
      source,
      sourceUrl,
      riskLevel: MILD_PATTERN.test(normalized) || /\/middlefinger(?:[/?#]|$)/iu.test(sourceUrl) ? 'mild' : 'safe'
    }
  }
}
