<template>
  <view class="page-download-history">
    <!-- 毛玻璃导航栏 -->
    <GlassNavBar title="下载历史" :show-back="true" />
...
const loadDownloads = async (reset = false) => {
  ...
  try {
    ...
    const result = await getDownloadRecords({ page: page.value, limit })
    const items = (result.list || []).map(d => ({
      ...d.material,
      id: d.material?.id || d.materialId,
      downloadId: d.id
    }))
